function [modelFile, trainLoss] = train_pirn_model_4(sampleFile, trainParams)
% PINN
...
% https://benmoseley.blog/my-research/so-what-is-a-physics-informed-neural-network/
    ...
    initTimes = 1:trainParams.initTimeStep:3; %start from 1 sec to 4 sec with 0.5 sec step 
    tTrain = [];
    xTrain = [];
    yTrain = [];
    for i = 1:numSamples
    ...
    numStates = 4;
     layers = [
        ...
        fullyConnectedLayer(trainParams.numNeurons, "Name",'fc1')
        ];
    
    % Add residual blocks
    for i = 1:trainParams.numLayers-1
        layers = [
            layers
            fullyConnectedLayer(trainParams.numNeurons, "Name",['fc',num2str(2*i)])
            reluLayer("Name",['relu', num2str(2*i)])
            fullyConnectedLayer(trainParams.numNeurons, "Name",['fc',num2str(2*i+1)])
    ...
    % Add skip connections for the residual blocks
    lgraph = connectLayers(lgraph, "fc1", "add1/in2");
    for i = 1:trainParams.numLayers-2
        lgraph = connectLayers(lgraph, ['add',num2str(i)], ['add',num2str(i+1),'/in2']);
    end
